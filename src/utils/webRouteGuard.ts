...
// it to a block list.
//
// Two exact-match exceptions beyond /p and /c: /auth/callback and
// /reset-password. Spero's real auth redirect URL is the native
// "speropay://auth/callback" scheme (see authDeepLink.ts) -- in ordinary
// use, a password-recovery or email-confirmation link never actually opens
// the web build at all. These two are allowed anyway, defensively: if a
// recovery link is ever opened somewhere the native scheme can't handle
// (an old device, a different browser, a future web fallback in Supabase's
// own redirect config), a merchant must still be able to finish resetting
// their password rather than dead-ending on a marketing page with no way
// forward. Neither screen exposes any merchant dashboard data.
const WEB_PUBLIC_PATH_PREFIXES = ['/p/', '/c/'];
